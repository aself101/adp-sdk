...
export class AdpHttpClient {
  ...
  constructor(config: ResolvedConfig) {
    ...
    try {
      ...
      throw new AdpAPIError('Cannot read mTLS certificate file. Ensure the certPath file exists and is readable. Set via certPath config or ADP_CERT_PATH env var.', ERROR_CODES.CONFIG_ERROR, undefined, undefined, undefined, err);
    }
    try {
      key = fs.readFileSync(config.keyPath);
    } catch (err) {
      throw new AdpAPIError('Cannot read mTLS private key file. Ensure the keyPath file exists and is readable. Set via keyPath config or ADP_KEY_PATH env var.', ERROR_CODES.CONFIG_ERROR, undefined, undefined, undefined, err);
    }
